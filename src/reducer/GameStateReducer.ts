...
import {Block, BlockType, Cell, Coordinates, Direction, GameLevelState,} from "../model/GameFieldModel";
...
export const GameStateReducer = (state: GameState, action: GameStateAction): GameState => {
    switch (action.type) {
        ...
        case "destroy_marked_cells":
            return destroyMarkedCells(state);
...
const spawnBlock = (state: GameState, nextBlock: BlockType) => {
    ...
    const blockSpawnPositioned = blockFactory(block.type, {
        col: block.anchor.col,
        row: state.gameField.rows + state.gameField.rows - 2 - getHighestCell(block).row
    });
...
const updateField = (state: GameState, updatedBlocks: Block[]) => {
    ...
    const newBlocks = updatedBlocks.map(({cells, ...rest}) => {
        return {
            ...rest,
            cells: cells!.map(({row, col, ...rest}) => {
                const shouldBeDestroyed = cellsToDestroy.some(({row: cellRow, col: cellCol}) => cellRow === row && cellCol === col);
                return {
                    ...rest,
                    row,
                    col,
                    isMarkedForDestruction: shouldBeDestroyed
                };
            }),
            isMarkedForDestruction: cells!.some(({row, col}) => cellsToDestroy.some(({row: cellRow, col: cellCol}) => cellRow === row && cellCol === col))
        }
    });
...
const destroyMarkedCells = (state: GameState) => {
    const cellsToDestroy = state.blocks.reduce((acc: Cell[], val) => [...acc, ...val.cells!], []).filter(({isMarkedForDestruction}) => isMarkedForDestruction);
    const blocksWithoutDestroyedCells = state.blocks.map(({cells, ...rest}) => {
            return {
                ...rest,
                cells: cells!.filter(({isMarkedForDestruction}) => !isMarkedForDestruction)
            };
        }
    );
